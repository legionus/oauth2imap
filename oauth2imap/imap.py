...
class Downstream:
    def __init__(self, s: socketserver.StreamRequestHandler):
        self.addr = s.client_address
        self.rfile = s.rfile
        self.wfile = s.wfile
...
class ImapTCPHandler(socketserver.StreamRequestHandler):
    def handle(self) -> None:
        ...
        try:
            ...
            up = Upstream(config["upstream"]["server"], config["upstream"]["port"])
            ...
            logger.debug("%s: connection error: %s", self.client_address, e)
...
def main(cmdargs: argparse.Namespace) -> int:
    ...
    oauth2imap.auth.logger = logger
    oauth2imap.oauth2.logger = logger
